...
import {
  Plus, Search, Trash2, Edit2, Ticket, X, XCircle,
  ChevronLeft, ChevronRight, RefreshCw, Copy,
  Calendar, CheckCircle2, AlertTriangle, Ban,
...
function getCouponStatus(coupon) {
  ...
  if (coupon.usedCount >= coupon.usageLimit) return { label: 'Exhausted', class: 'badge-exhausted' };
...
export default function AdminCoupons() {
  ...
  const deleteCoupon = async (id) => {
    try {
      ...
      const targetPage = newTotal <= (pagination.page - 1) * pagination.limit
  ...
  const statCards = stats ? [
    { label: 'Total Coupons', value: stats.total, color: 'bg-blue-light', iconColor: 'icon-blue', icon: Ticket },
    { label: 'Active', value: stats.active, color: 'bg-green-light', iconColor: 'icon-green', icon: CheckCircle2 },
    { label: 'Inactive', value: stats.inactive, color: 'bg-gray-100', iconColor: 'text-gray-500', icon: Ban },
    { label: 'Expired', value: stats.expired, color: 'bg-red-light', iconColor: 'icon-red', icon: AlertTriangle },
    { label: 'Exhausted', value: stats.exhausted, color: 'bg-orange-light', iconColor: 'icon-orange', icon: XCircle },
  ] : [];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreateModal} className="flex items-center gap-2 bg-red-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-red-700 transition-colors shadow-lg shadow-red-900/15">
      ...
      <div className="bg-white rounded-xl p-4 mb-6 shadow-sm border border-gray-100">
        <div className="flex flex-wrap items-center gap-3">
          <form onSubmit={handleSearch} className="relative flex-1 min-w-[200px]">
            ...
            />
            {searchTerm && (
              <button type="button" onClick={() => { setSearchTerm(''); fetchCoupons(1); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                <X size={16} />
              </button>
            )}
          </form>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Min Order</th>
                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Validity</th>
                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Usage</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="admin-empty-state py-12">
                    ...
                    <p className="text-gray-500 font-bold uppercase tracking-widest text-xs mb-1">No coupons found</p>
                    ...
                    <tr key={coupon._id} className="hover:bg-gray-50 transition-colors group">
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="font-black text-slate-800 text-sm uppercase tracking-wide">{coupon.code}</span>
                          <button
                        ...
                        {coupon.description && (
                          <p className="text-xs text-gray-400 mt-0.5 line-clamp-1 max-w-[220px]">{coupon.description}</p>
                        )}
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-1.5">
                          <span className={`admin-icon-small-wrap ${coupon.discountType === 'percentage' ? 'bg-green-light' : 'bg-blue-light'}`}>
                            {coupon.discountType === 'percentage'
                              ? <Percent size={14} className="icon-green" />
                              : <IndianRupee size={14} className="icon-blue" />
                            }
                          </span>
                          <span className="font-bold text-slate-800">{getDiscountLabel(coupon)}</span>
                        </div>
                        {coupon.maxDiscount && coupon.discountType === 'percentage' && (
                          <p className="text-[10px] text-gray-400 mt-0.5 ml-9">Max discount ₹{coupon.maxDiscount}</p>
                        )}
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-xs text-gray-400">→ {formatDate(coupon.validUntil)}</span>
                        </div>
                      </td>
      ...
      {/* Create / Edit Modal */}
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 z-[10000] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden ring-1 ring-black/5 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            {/* Modal Header */}
            <div className="p-6 bg-slate-900 text-white flex justify-between items-center shrink-0">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5">{editingCoupon ? 'Update coupon details' : 'Set up a new promotional code'}</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
              <div className="grid grid-cols-2 gap-5">
                <div className="col-span-2 sm:col-span-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Coupon Code</label>
                  <input
                    required
                    value={formData.code}
                ...
                <div className="col-span-2 sm:col-span-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Discount Type</label>
                  <div className="flex gap-2 p-1 bg-slate-50 rounded-xl border border-slate-100">
                    <button
                      type="button"
                      onClick={() => setFormData({ ...formData, discountType: 'percentage' })}
                      className={`flex-1 py-2.5 rounded-lg text-xs font-black transition-all ${
                        formData.discountType === 'percentage' ? 'bg-white shadow-sm text-red-600' : 'text-slate-400 hover:text-slate-600'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-lg text-xs font-black transition-all ${
                        formData.discountType === 'fixed' ? 'bg-white shadow-sm text-red-600' : 'text-slate-400 hover:text-slate-600'
                      }`}
                    >
                      <IndianRupee size={14} className="inline mr-1" />Fixed Amount
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                {formData.discountType === 'percentage' && (
                  <div>
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Max Discount (₹)</label>
                    <input
                      type="number"
                      min="0"
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Usage Limit</label>
                  <input
                    type="number"
                    required
                    min="1"
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Min Order Value (₹)</label>
                  <input
                    type="number"
                    min="0"
              ...
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Description</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Valid From</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1.5">Valid Until</label>
                  <input
                    type="date"
                    required
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 z-[10001] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 ring-1 ring-black/5" onClick={(e) => e.stopPropagation()}>
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Are you sure you want to delete <strong className="text-slate-700 uppercase">{deleteConfirm.code}</strong>?
