...
import { Save, CreditCard, Smartphone, Wallet, Eye, EyeOff, CheckCircle, XCircle, IndianRupee, Shield, Globe } from 'lucide-react';
import api from '../../lib/api';
import toast from 'react-hot-toast';

const METHODS = [
  { key: 'card',        label: 'Credit / Debit Cards', icon: CreditCard,   color: 'text-blue-600', bg: 'bg-blue-100',  desc: 'Visa, Mastercard, Rupay & more' },
  { key: 'google_pay',  label: 'Google Pay',           icon: Smartphone,   color: 'text-green-600', bg: 'bg-green-100', desc: 'Pay using Google Pay UPI', testField: { label: 'Test UPI ID', configKey: 'googlePayTestId', placeholder: 'success@razorpay' } },
  { key: 'phonepe',     label: 'PhonePe',              icon: Smartphone,   color: 'text-purple-600',bg: 'bg-purple-100',desc: 'Pay using PhonePe UPI',  testField: { label: 'Test UPI ID', configKey: 'phonePeTestId', placeholder: 'success@razorpay' } },
  { key: 'paytm_wallet',label: 'Paytm Wallet',         icon: Wallet,       color: 'text-blue-500', bg: 'bg-blue-50',   desc: 'Pay using Paytm balance, UPI & cards' },
];

const TEST_CARDS = [
  { type: 'visa',         label: 'Visa',         number: '4111 1111 1111 1111', expiry: '12/28', cvv: '123', holder: 'Test User' },
  { type: 'mastercard',   label: 'Mastercard',   number: '5555 5555 5555 4444', expiry: '12/28', cvv: '123', holder: 'Test User' },
  { type: 'rupay',        label: 'Rupay',        number: '6069 8888 8888 8888', expiry: '12/28', cvv: '123', holder: 'Test User' },
  { type: 'amex',         label: 'Amex',         number: '3782 822463 10005',   expiry: '12/28', cvv: '1234', holder: 'Test User' },
  { type: 'maestro',      label: 'Maestro',      number: '5000 0000 0000 0610', expiry: '12/28', cvv: '123', holder: 'Test User' },
];
    ...
    <div>
      ...
      <div className="grid gap-6 max-w-2xl">
        ...
        {METHODS.map((method) => {
          ...
          if (method.key === 'card') {
            return (
              <div key={method.key} className={`admin-widget-card transition ${!isEnabled ? 'opacity-60' : ''}`}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isEnabled && <span className="text-[10px] font-semibold text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{config.currency}</span>}
                    </div>
                  </div>
                  <button
                ...
                {isEnabled && (
                  <div className="ml-[52px] space-y-4">
                    <div>
                      <p className="text-xs font-medium text-gray-500 mb-2">Accepted Card Networks</p>
                      <div className="flex flex-wrap gap-2">
                        {[
                          { name: 'Visa', color: 'bg-blue-50 text-blue-700 border-blue-200' },
                          { name: 'Mastercard', color: 'bg-orange-50 text-orange-700 border-orange-200' },
                          { name: 'Rupay', color: 'bg-green-50 text-green-700 border-green-200' },
                          { name: 'American Express', color: 'bg-purple-50 text-purple-700 border-purple-200' },
                          { name: 'Maestro', color: 'bg-gray-50 text-gray-700 border-gray-200' },
                        ].map((card) => (
                          <span key={card.name} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md border text-[11px] font-semibold ${card.color}`}>
                            <CreditCard size={11} />
                            {card.name}
                          </span>
                        ))}
                    ...
                    <div className="border-t border-gray-100 pt-4">
                      ...
                      <div className="mb-3">
                        <label className="text-[11px] font-medium text-gray-400 block mb-1 uppercase tracking-wider">Select Card</label>
                        <select
                      ...
                      <div className="space-y-2.5">
                        <div>
                          <label className="text-[11px] font-medium text-gray-400 block mb-1 uppercase tracking-wider">Cardholder Name</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="text-[11px] font-medium text-gray-400 block mb-1 uppercase tracking-wider">Card Number</label>
                          <input
                            type="text"
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="text-[11px] font-medium text-gray-400 block mb-1 uppercase tracking-wider">Expiry Date</label>
                            <input
                              type="text"
                              value={config.testCardExpiry || ''}
                              onChange={(e) => setConfig({ ...config, testCardExpiry: e.target.value })}
                              placeholder="MM/YY"
                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-red-500"
                            />
                          </div>
                          <div>
                            <label className="text-[11px] font-medium text-gray-400 block mb-1 uppercase tracking-wider">CVV</label>
                            <input
                              type="password"
            ...
            <div key={method.key} className={`admin-widget-card transition ${!isEnabled ? 'opacity-60' : ''}`}>
              ...
              {isEnabled && method.testField && (
                <div className="ml-[52px]">
                  <label className="text-xs font-medium text-gray-500 block mb-1">{method.testField.label}</label>
                  <input
                    type="text"
                    value={config[method.testField.configKey] || ''}
                    onChange={(e) => setConfig({ ...config, [method.testField.configKey]: e.target.value })}
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">Use <span className="font-mono">success@razorpay</span> to simulate successful payment</p>
                </div>
              )}
              {!isRazorpayConfigured && (
                <p className="text-xs text-amber-600 flex items-center gap-1 mt-2">
